...
//		return entry == null ? null : findLeafPage(tid, dirtypages, entry.getRightChild(), perm, f);
		...
		//mine
		BTreePageId tmpid = pid;
		while (tmpid.pgcateg() != BTreePageId.LEAF){
			BTreeInternalPage page = (BTreeInternalPage)getPage(tid,dirtypages,tmpid,Permissions.READ_ONLY);
			Iterator<BTreeEntry> iterator = page.iterator();
			while (iterator.hasNext()){
				BTreeEntry next = iterator.next();
				if (f == null){
					tmpid = next.getLeftChild();
					break;
				}else{
					...
					tmpid = next.getRightChild();
				}
			}

		}
		return (BTreeLeafPage)getPage(tid,dirtypages,tmpid,perm);
	}
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int offset = page.getNumTuples()>>1;
		...
		for(int i = tmp.size()-1;i>=0;i--) newPage.insertTuple(tmp.get(i));
		newPage.setLeftSiblingId(page.getId());
		BTreePageId rightId = page.getRightSiblingId();
		BTreeLeafPage right = null;
		if(rightId != null){
			right = (BTreeLeafPage) getPage(tid, dirtypages, rightId, Permissions.READ_WRITE);
		...
		//dirtypages.put(rightId, right);

		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		BTreeEntry entry = new BTreeEntry(next.getField(page.keyField), page.pid, newPage.getId());
		parent.insertEntry(entry);
		dirtypages.put(parent.getId(), parent);

		updateParentPointers(tid, dirtypages, parent);
		return field.compare(Op.LESS_THAN, entry.getKey())?page : newPage;
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int offset = page.getNumEntries()>>1;
		Iterator<BTreeEntry> iterator = page.reverseIterator();
		List<BTreeEntry> tmp = new ArrayList<>();
		...
		for(int i = tmp.size()-1;i>=0;i--) newPage.insertEntry(tmp.get(i));

		BTreeEntry mid = iterator.next();
		page.deleteKeyAndRightChild(mid);
		mid.setLeftChild(page.pid);
		mid.setRightChild(newPage.pid);
		...
		dirtypages.put(parent.getId(), parent);
		updateParentPointers(tid, dirtypages, parent);
		return field.compare(Op.LESS_THAN, mid.getKey())? page : newPage;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int cur = page.getNumTuples();
		int sib = sibling.getNumTuples();
		int t = cur+sib >> 1;
		Iterator<Tuple> iterator;
		if (isRightSibling){
			iterator = sibling.iterator();

		}else iterator = sibling.reverseIterator();
		while(cur++ < t){
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int cur = page.getNumEntries();
		int sib = leftSibling.getNumEntries();
		int t = cur+sib>>1;
		Iterator<BTreeEntry> iterator = leftSibling.reverseIterator();
		BTreeEntry rightOfLeft = iterator.next();
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), rightOfLeft.getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(entry);
		//leftSibling.deleteKeyAndRightChild(rightOfLeft);
		cur++;

		while (cur++ < t){
			leftSibling.deleteKeyAndRightChild(rightOfLeft);
			page.insertEntry(rightOfLeft);
			rightOfLeft = iterator.next();
		}
		BTreeEntry mid = rightOfLeft;
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int cur = page.getNumEntries();
		int sib = rightSibling.getNumEntries();
		int t = cur+sib>>1;
		Iterator<BTreeEntry> iterator = rightSibling.iterator();
		BTreeEntry leftOfRight = iterator.next();
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), leftOfRight.getLeftChild());
		page.insertEntry(entry);
		//rightSibling.deleteKeyAndLeftChild(leftOfRight);
		cur++;

		while (cur++ < t){
			rightSibling.deleteKeyAndLeftChild(leftOfRight);
			page.insertEntry(leftOfRight);
			leftOfRight = iterator.next();
		}
		BTreeEntry mid = leftOfRight;
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if (rightSiblingId != null){
			BTreeLeafPage right = (BTreeLeafPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
			right.setLeftSiblingId(leftPage.getId());
		}
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
