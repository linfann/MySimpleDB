...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        if (!joinOp.equals(Predicate.Op.EQUALS)) return card1*card2*3/10;
        if (t2pkey&&t1pkey)return Math.min(card1,card2);
        if (!t2pkey&&!t1pkey)return Math.max(card1,card2);
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        //System.out.println(joins.size() + "  ------------------------------- " + bestCostCard.plan.size());
